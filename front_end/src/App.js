...
class App extends React.Component {
    ...
    handleUploadFileSubmit = async (event) => {
        ...
        const response = await fetch(window.location.origin + "/api/upload-script/", {
            method: 'POST',
            body: formData,
        });
        if (response.ok) {
    ...
    handleCharactersListSubmit = async (event) => {
        event.preventDefault();
        this.setState({ loading: true })
        const response = await fetch(window.location.origin + "/api/result-with-user-gender-by-title/", {
            method: 'POST',
